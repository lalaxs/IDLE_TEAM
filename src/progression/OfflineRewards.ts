...
export function calculateOfflineReward(
  ...
  if (rawMinutes < 5 || highestStage < 3) return { minutes: 0, gold: 0, gearCount: 0 };
  const minutes = Math.min(480, rawMinutes);
  const gold = (20 + highestStage * 12) * minutes;
  ...
  return { minutes, gold, gearCount: floor + (pseudo < fraction ? 1 : 0) };
}
